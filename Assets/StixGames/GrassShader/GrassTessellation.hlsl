...
inline float CalcTessFactor (float3 densityCenter, float3 wpos0, float3 wpos1)
{
	...
	dist = pow(max(dist - _DensityFalloffStart, 0.0) * _DensityFalloffScale, _DensityFalloffPower);

	//The target distance between blades of grass
	fixed targetLen = _TargetDensity * max(dist, 1.0);

	return len / targetLen;
}

inline fixed4 EdgeLengthBasedTessCull (float3 densityCenter, float3 v0, float3 v1, float3 v2)
{
	fixed4 tess;
...
HS_CONSTANT_OUTPUT HSConstant(InputPatch<tess_appdata, 3> ip)
{
	...
	fixed t = pow(2,_TargetTessellation);
	fixed4 rTess = fixed4(t, t, t, t);
	#else
	fixed4 rTess = EdgeLengthBasedTessCull(ip[0].cameraPos, ip[0].vertex.xyz, ip[1].vertex.xyz, ip[2].vertex.xyz);
	#endif

	fixed tess = step(1.0, rTess.w) * nextPow2(rTess.w);
...
GS_INPUT domainShader(HS_CONSTANT_OUTPUT input, float3 tLoc : SV_DomainLocation, const OutputPatch<tess_appdata, 3> patch)
{
...
#if defined(SHADER_API_GLCORE)
	fixed prevTess = input.mTess * 0.5;
#else
	...
	float r1 = sqrt(rand(randCalcPos.xz + float2(839,-919)));
	float r2 = rand(randCalcPos.xz + float2(-113, 13));
